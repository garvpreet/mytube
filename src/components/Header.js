...
import ytLogoDefault from "../assets/images/yt-logo.png";
import ytLogoDark from "../assets/images/yt-logo-dark.png";
import { useDispatch, useSelector } from "react-redux";
import { closeMenu, setMenu, toggleMenu } from "../utils/SideMenuSlice";
...
const Header = () => {
  ...
  useEffect(() => {
    // Hide sidebar intially for small screens
    if (window.innerWidth < 768) dispatcher(closeMenu());
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (suggestions && suggestions[queryText]) {
  ...
  const getData = async () => {
    const data = await fetch(SEARCH_API + queryText);
    const json = await data.json();
    setSearchSuggestions(json[1]);
    dispatcher(addCacheItem({ [queryText]: json[1] }));
  };
  ...
  return (
    <div className="sticky top-0 grid grid-flow-col py-3 md:px-3 px-1 bg-white dark:bg-slate-800 w-full">
      ...
      <div
        className={`flex col-span-2 md:col-span-1 items-center mx-4 ${
      ...
      >
        <div className={`group flex flex-row `}>
          <input
            value={queryText}
            className={`${
              smSearch ? "w-full" : "w-3/5 max-sm:hidden"
            } md:w-2/3 border border-gray-400 rounded-l-full py-1 pl-3 md:pl-5 group-focus-within:border-sky-300 dark:bg-slate-800 dark:text-white`}
            type="text"
            placeholder="Search"
            onChange={(e) => {
          ...
          <button
            ...
            } rounded-r-full md:py-2 px-2 md:px-5 flex justify-center items-center  hover:bg-gray-100`}
            onClick={() => {
              const screenWidth = window.innerWidth;
          ...
          >
            <GoSearch className="text-xl max-sm:mt-1 dark:text-white" />
          </button>
        </div>
            ...
            <div className=" py-2">
              <ul>
                {searchSuggestions.map((s, index) => (
                  ...
                  >
                    <Link to={"/search?q=" + s} onClick={() => setQueryText(s)}>
